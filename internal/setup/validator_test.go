...
func TestValidate_InvalidRuntime(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "must be one of") {
		t.Errorf("Expected runtime validation error, got: %v", err)
	}
}

func TestValidate_ValidRuntime(t *testing.T) {
	config := &SetupConfig{
		Version: 1,
		Provision: []Step{
			{
...
func TestValidate_InvalidDatabase(t *testing.T) {
	...
	err := Validate(config)
	if err == nil {
		t.Fatal("Expected validation error for invalid database")
	}
...
func TestValidate_ValidDatabase(t *testing.T) {
	config := &SetupConfig{
		Version: 1,
		Provision: []Step{
			{
...
func TestValidate_InvalidToolChecksum(t *testing.T) {
	config := &SetupConfig{
		Version: 1,
		Setup: []Step{
			{
				Name: "Invalid tool checksum",
				EnsureTool: &EnsureTool{
					Name: "test-tool",
					Install: &ToolInstallConfig{
						FallbackURL: "https://example.com/tool.zip",
	...
	err := Validate(config)
	if err == nil {
		t.Fatal("Expected validation error for invalid checksum")
	}

	if !strings.Contains(err.Error(), "sha256:hexdigest") {
		t.Errorf("Expected checksum format error, got: %v", err)
	}
}

func TestValidate_MissingChecksum(t *testing.T) {
	config := &SetupConfig{
		Version: 1,
		Setup: []Step{
			{
	...
	err := Validate(config)
	if err == nil {
		t.Fatal("Expected validation error for missing checksum")
	}

	if !strings.Contains(err.Error(), "requires checksum") {
		t.Errorf("Expected checksum requirement error, got: %v", err)
	}
}
